...
from Practicas.bab_scheme import BabPartialSolution, BabSolver, Solution
...
def binpacking_solve(objects: List[int], capacity: int):
    class BinPackingBabPS(BabPartialSolution):
        ...
        def calc_opt_bound(self) -> Union[int, float]:
            contenedores = deepcopy(self.container_weights)
            menorQueQueda = capacity
            mayorQueQueda = 0
            iterador = []
            for i in range(self.n, len(objects)):
                if objects[i]<menorQueQueda: menorQueQueda = objects[i]
            for i in range(len(contenedores)):
                if contenedores[i]>=menorQueQueda:
                    iterador.append(i)
                if contenedores[i]>mayorQueQueda:
                    mayorQueQueda=contenedores[i]
            sumahuecos = len(iterador)*capacity
            for i in iterador:
                sumahuecos-=contenedores[i]
            grandes=[]
            pequenos = []
            for i in range(self.n, len(objects)):
                if objects[i] > mayorQueQueda:
                    grandes.append(objects[i])
                else:
                    pequenos.append(objects[i])
            sumamedios = 0
            sumagrandes = 0
            for i in pequenos: sumamedios+=i
            for i in grandes: sumagrandes+=i
            if sumahuecos<sumamedios: sumamedios-=sumahuecos
            else: sumamedios = 0
            sumagrandes+=sumamedios
            cantidadnueva = ceil(sumagrandes/capacity)
            return len(self.container_weights)+ cantidadnueva
        ...
        def calc_pes_bound(self) -> Union[int, float]:
            contenedores = list(deepcopy(self.container_weights))
            copiasolucion = deepcopy(self.decisions)
            for i in range(self.n, len(objects)):
                for j in range(len(contenedores)+1):
                    if j < len(contenedores):
                        if objects[i]+contenedores[j] <=capacity:
                            copiasolucion+=(j,)
                            contenedores[j]+=objects[i]
                            break
                    else:
                        copiasolucion+=((j),)
                        contenedores+=(objects[i],)
            return len(contenedores)  # AHORA ES DEMASIADO PESIMISTA
